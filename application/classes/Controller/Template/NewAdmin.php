...
abstract class Controller_Template_NewAdmin extends Controller_Template {

    // Define the template to use
    public $template = 'new_admin/layout/layout';
    ...
    public function __construct(Request $request, Response $response) {
        ...
        if (!$user) {
           $this->redirect('NewHome', 302);
            ...
            if (!in_array($this->user_online->user_groub, [1,2,4,5])) {
                $this->redirect('/NewHome', 302);
            }
        }
    ...
    public function before() {
        ...
        if($this->user_online && $this->user_online->user_groub == 3){
            ...
            $this->template->Waiting_courses = ORM::factory('Students_Sections')
                                                    ->and_where_open()
                                                        ->or_where('state','!=',4)
                                                        ->or_where('state','=',NULL)
                                                    ->and_where_close()
        ...
        if($this->user_online){
            $this->template->notifications = Common_Notifications::GetUserNotifications($this->user_online);
            $this->template->mode = ($this->user_online->dark_mode == TRUE)? "dark" : "light"; 
            $this->template->layout->mode = ($this->user_online->dark_mode == TRUE)? "dark" : "light"; 
            $this->template->other_mode = ($this->template->mode == 'dark')? 'light' : 'dark';
        }
