...
class Common_Statistics_Exams {
    ...
    public static function GetTeacherExamsDetails($teacher,$from_date, $to_date){
        ...
        foreach($teacher_exams as $teacher_exam){
            ...
            $total_students_for_all_exams += $exam_statistic['total_students'];
            $total_succeed_students_for_all_exams += $exam_statistic['succeeded_students'];
        }
        $results["total_students"]  = $total_students;
        $results["succeeded_students"]  = $succeeded_students;
        $results["percentage"]  = ($total_student > 0)? $total_student : 0;
