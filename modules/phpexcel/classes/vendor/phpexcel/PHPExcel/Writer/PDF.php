...
$k_path_url = dirname(__FILE__) . '/mpdf';
...
class PHPExcel_Writer_PDF extends PHPExcel_Writer_HTML implements PHPExcel_Writer_IWriter {
	...
	public function save($pFilename = null) {
		...
		if (is_null($this->getSheetIndex())) {
			...
			$orientation = ($this->_phpExcel->getSheet($this->getSheetIndex())->getPageSetup()->getOrientation() == 'L') ? 'L' : 'P';
        ...
        $dir = $session->get('dir');
        $mpdf = new mPDF('ar', $paperSize, 9, 'xbriyaz',15,15,16,16,9,9,'L');
        if($dir === "rtl"){
        ...
        // Write to file
		fwrite($fileHandle, $mpdf->output($pFilename, 'S'));
